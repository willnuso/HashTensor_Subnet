...
def upgrade() -> None:
    ...
    op.add_column(
        "hotkey_worker",
        sa.Column("registration_time_int", sa.BigInteger(), nullable=True),
    )
    ...
    rows = list(
        conn.execute(
            sa.text(
                "SELECT worker, hotkey, registration_time, signature FROM hotkey_worker"
            )
        )
    )
    for worker, hotkey, registration_time, signature in rows:
        ...
        for offset in range(-5, 6):  # Try ±5 microseconds
            ...
            reg_json = json.dumps(
                reg_dict, sort_keys=True, separators=(",", ":")
            )
            if verify_signature(hotkey, reg_json, signature):
                reg_time_int = int(candidate_float * 1_000_000)
                conn.execute(
                    sa.text(
                        "UPDATE hotkey_worker SET registration_time_int = :reg_time_int WHERE worker = :worker"
                    ),
                    {"reg_time_int": reg_time_int, "worker": worker},
                )
                print(
                    f"[OK] Worker: {worker} | Restored: {candidate_float} | Int: {reg_time_int} | Offset: {offset}"
                )
                found = True
                break
        if not found:
            ...
            print(
                f"[WARN] Worker: {worker} | Used stored value: {stored} | Int: {reg_time_int} (signature may not verify)"
            )
